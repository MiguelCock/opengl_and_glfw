...
Entity *entReadFile(char *filename)
{
    ...
    while (ent->p3List == NULL) ent->p3List = (Point3 *)malloc(ent->pointsAmount * sizeof(Point3));

    for (int i = 0; i < ent->pointsAmount; i++)
    {
        printf("%d: ", i + 1);

        if (fread(&ent->p3List[i].x, sizeof(double), 1, file) < 0) perror("error\n");
        if (fread(&ent->p3List[i].y, sizeof(double), 1, file) < 0) perror("error\n");
        ent->p3List[i].w = 1;
